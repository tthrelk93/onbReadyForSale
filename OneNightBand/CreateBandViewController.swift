                ...
                storageRef.putData(uploadData, metadata: nil, completion: { (metadata, error) in
                    ...
                    if let bandImageUrl = metadata?.downloadURL()?.absoluteString {
                        ...
                        self.thisBand.setValuesForKeys(values)
                         ...
                         self.bandID = bandReferenceAnyObject
                        let ref = Database.database().reference()
                        print("searchType: \(self.searchType)")
                        if self.searchType == "wanted"{
                            let recipient = self.ref.child("users").child(self.artistUID).child("wantedAds")
                            let tempID = recipient.childByAutoId()
                            
                            self.wantedAd["bandID"] = bandReferenceAnyObject
                            //values["instrumentNeeded"] = self.buttonName
                            self.wantedAd["date"] = "n/a"
                            self.wantedAd["inviteKey"] = String(describing: tempID.key)
                            self.wantedAd["bandType"] = "band"//self.bandType
                            //values["artistCount"] = self.artistCount
                            //values["bandType"] = self.bandType
                            self.wantedAd["bandName"] = self.bandNameTextField.text
                            tempID.updateChildValues(self.wantedAd, withCompletionBlock: {(err, ref) in
                                if err != nil {
                                    print(err as Any)
                                    return
                                }
                                ...
                                self.inviteDict["bandID"] = bandReferenceAnyObject
                                //values["instrumentNeeded"] = self.buttonName
                                self.inviteDict["date"] = "n/a"//self.sessionDate
                                self.inviteDict["inviteKey"] = String(describing: tempID.key)
                                self.inviteDict["bandType"] = "band"//self.bandType
                                //values["artistCount"] = self.artistCount
                                //values["bandType"] = self.bandType
